...
// single length percentage or no parcentage value.
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      category: 'Possible Errors or warnings',
      recommended: true,
    },
    schema: [],
    messages: {
          ...
          if (
            ...
            if (validData[key]) {
              ...
              const multipleValueRegex = new RegExp(
                `^(${lengthValuePattern})( (?!\\s)(${lengthValuePattern})){0,${
                  valueCountMap[key] - 1
                }}$`,
              )
              ...
              const urlPattern = `(?:${urlString}|${varString}\\s*)?`
              ...
              function createBorderImageRegex() {
                ...
                const imageSource = `${imageRegex.source.slice(
                  1,
                  -1,
                )}|${varString}` // Image source or var()
              ...
              const blurFunctionRegex = `blur\\(\\s*(${lengthPattern}|${calcString}|${clampString}|${minString}|${maxString})\\s*\\)`
              const angleFunctionRegex = `hue-rotate\\(\\s*${anglePattern}\\s*\\)`

              function isValidFilter(value) {
                if (new RegExp(`^${urlString}$`).test(value)) {
                  return true
                }

                const dropShadowFullPattern = 'drop-shadow\\([^)]*\\)'

                const backdropFilterPattern = new RegExp(
                  `^(?:${filterNumFunctionPattern}|${blurFunctionRegex}|${angleFunctionRegex}|${dropShadowFullPattern}|${urlString}|${varString})(?:\\s+(?:${filterNumFunctionPattern}|${blurFunctionRegex}|${angleFunctionRegex}|${dropShadowFullPattern}|${urlString}|${varString}))*$`,
                )

                if (!backdropFilterPattern.test(value)) return false

                const filterFunctions = value.match(
                  /\b(brightness|contrast|grayscale|invert|opacity|sepia|saturate|blur|hue-rotate|drop-shadow|urlString)\([^)]*\)/g,
                )

                if (!filterFunctions) return false

                return filterFunctions.every((func) => {
                  if (/^drop-shadow\(/.test(func)) {
                    const dropShadowRegex = new RegExp(
                      `^drop-shadow\\(\\s*(?:${colorSource}|${lengthValuePattern})(?:\\s+(?:${colorSource}|${lengthValuePattern})){2,3}\\s*\\)$`,
                    )
                    return dropShadowRegex.test(func)
                  }

                  if (
                    /^(brightness|contrast|grayscale|invert|opacity|sepia|saturate)\(/.test(
                      func,
                    )
                  ) {
                    return new RegExp(filterNumFunctionPattern).test(func)
                  }

                  if (/^blur\(/.test(func)) {
                    return new RegExp(blurFunctionRegex).test(func)
                  }

                  if (/^hue-rotate\(/.test(func)) {
                    return new RegExp(angleFunctionRegex).test(func)
                  }

                  if (/^url\(/.test(func)) {
                    return new RegExp(urlString).test(func)
                  }
              ...
              const repeatFullSet = `(${repeatKeyword}|${backgroundRepeatRegex.source.slice(
                1,
                -1,
              )})`
              ...
              if (translateProperties.includes(key)) {
                ...
                if (!isValidFilter(value) && globalValue) {
                  report()
                }
